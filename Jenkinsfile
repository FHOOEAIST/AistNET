pipeline {
    agent {
        docker { 
            image 'aist.fh-hagenberg.at:18444/repository/docker-util/aist-python:3.7.1'
            args '-u root:sudo'
            alwaysPull true
        }
    }
            ...
            steps {
                script {
                    ...
                    catchError(buildResult: 'UNSTABLE', stageResult: 'UNSTABLE') {
                        ...
                        pylint src/aistnet/ --reports=n --msg-template="{path}:{line}: [{msg_id}({symbol}), {obj}] {msg}" > .tox/pylint.txt
                        """
                    }
                }
            }
    ...
    post {
        always {
            sh """
            sonar-scanner \
                -Dsonar.projectKey=aistnet \
                -Dsonar.projectName=aistnet \
                -Dsonar.language=py \
